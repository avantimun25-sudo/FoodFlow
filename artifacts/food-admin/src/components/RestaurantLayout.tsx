    ...
    <div className="flex min-h-screen w-full overflow-hidden" style={{ background: "hsl(var(--background))" }}>
      <aside className="w-72 hidden md:flex flex-col border-r border-orange-500/10 relative z-10 h-screen sticky top-0 flex-shrink-0"
        ...
        <div className="flex-1 overflow-y-auto py-6 px-4 space-y-1">
          <p className="px-4 text-xs font-semibold uppercase tracking-wider mb-4" style={{ color: "rgba(249,115,22,0.6)" }}>
            Navigation
          </p>
          {navItems.map((item) => {
        ...
        <div className="p-4 border-t border-orange-500/10">
          <div className="rounded-2xl p-4 flex items-center gap-3 border"
            style={{ background: "rgba(249,115,22,0.08)", borderColor: "rgba(249,115,22,0.15)" }}>
      ...
      <main className="flex-1 flex flex-col min-w-0 relative overflow-hidden">
        ...
        <header className="h-20 flex items-center justify-between px-8 border-b sticky top-0 z-20 backdrop-blur-md"
          style={{ borderColor: "rgba(249,115,22,0.1)", background: "rgba(10,8,6,0.85)" }}>
          <div>
            <h2 className="text-xl font-bold text-white">{pageTitle}</h2>
            <p className="text-xs" style={{ color: "rgba(249,115,22,0.7)" }}>{restaurantName}</p>
          </div>
          <div className="flex items-center gap-3">
